#ifndef HAVE_PTHREADS_OPENSSL_XP_SSL
#define HAVE_PTHREADS_OPENSSL_XP_SSL
...
#define PTHREADS_STREAM_CRYPTO_IS_CLIENT            (1<<0)
#define PTHREADS_STREAM_CRYPTO_METHOD_SSLv2         (1<<1)
#define PTHREADS_STREAM_CRYPTO_METHOD_SSLv3         (1<<2)
#define PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_0       (1<<3)
#define PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_1       (1<<4)
#define PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_2       (1<<5)
#define PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_3       (1<<6)

#ifndef OPENSSL_NO_SSL3
#define HAVE_SSL3 1
#define PTHREADS_OPENSSL_MIN_PROTO_VERSION PTHREADS_STREAM_CRYPTO_METHOD_SSLv3
#else
#define PTHREADS_OPENSSL_MIN_PROTO_VERSION PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_0
#endif
#define PTHREADS_OPENSSL_MAX_PROTO_VERSION PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_3

#define PTHREADS_SSL_MAX_VERSION_LEN 32
...
#define PTHREADS_GET_VER_OPT(name) \
	(pthreads_stream_has_context(threaded_stream) && (val = pthreads_stream_context_get_option(pthreads_stream_get_context(threaded_stream), "ssl", name)) != NULL)
#define PTHREADS_GET_VER_OPT_STRING(name, str) \
	if (PTHREADS_GET_VER_OPT(name)) { convert_to_string_ex(val); str = Z_STRVAL_P(val); }
#define PTHREADS_GET_VER_OPT_LONG(name, num) \
	if (PTHREADS_GET_VER_OPT(name)) { num = zval_get_long(val); }

/* Used for peer verification in windows */
#define PTHREADS_X509_NAME_ENTRY_TO_UTF8(ne, i, out) \
	ASN1_STRING_to_UTF8(&out, X509_NAME_ENTRY_get_data(X509_NAME_get_entry(ne, i)))

#if PTHREADS_OPENSSL_API_VERSION < 0x10100
static RSA *pthreads_openssl_tmp_rsa_cb(SSL *s, int is_export, int keylength);
#endif

extern pthreads_stream_t* pthreads_openssl_get_stream_from_ssl_handle(const SSL *ssl);
extern zend_string* pthreads_openssl_x509_fingerprint(X509 *peer, const char *method, zend_bool raw);
extern int pthreads_openssl_get_ssl_stream_data_index();
static struct timeval pthreads_openssl_subtract_timeval(struct timeval a, struct timeval b);
static int pthreads_openssl_compare_timeval(struct timeval a, struct timeval b);
static size_t pthreads_openssl_sockop_io(int read, pthreads_stream_t *threaded_stream, char *buf, size_t count);

const pthreads_stream_ops pthreads_openssl_socket_ops;

/* Certificate contexts used for server-side SNI selection */
typedef struct _pthreads_openssl_sni_cert_t {
	char *name;
	SSL_CTX *ctx;
} pthreads_openssl_sni_cert_t;

/* Provides leaky bucket handhsake renegotiation rate-limiting  */
typedef struct _pthreads_openssl_handshake_bucket_t {
	zend_long prev_handshake;
	zend_long limit;
	zend_long window;
	float tokens;
	unsigned should_close;
} pthreads_openssl_handshake_bucket_t;

#ifdef HAVE_TLS_ALPN
/* Holds the available server ALPN protocols for negotiation */
typedef struct _pthreads_openssl_alpn_ctx_t {
	unsigned char *data;
	unsigned short len;
} pthreads_openssl_alpn_ctx;
#endif
 ...
 * */
typedef struct _pthreads_openssl_netstream_data_t {
	pthreads_netstream_data_t s;
	SSL *ssl_handle;
	SSL_CTX *ctx;
	...
	pthreads_openssl_handshake_bucket_t *reneg;
	pthreads_openssl_sni_cert_t *sni_certs;
	unsigned sni_cert_count;
#ifdef HAVE_TLS_ALPN
	pthreads_openssl_alpn_ctx alpn_ctx;
#endif
	char *url_name;
	unsigned state_set:1;
	unsigned _spare:31;
} pthreads_openssl_netstream_data_t;
...
static int pthreads_openssl_is_http_stream_talking_to_iis(pthreads_stream_t *threaded_stream) { /* {{{ */
	...
	if (threaded_val != NULL &&
		stream->wrapper &&
		strcasecmp(PTHREADS_FETCH_STREAMS_WRAPPER(stream->wrapper)->wops->label, "HTTP") == 0) {
...
#define SERVER_GOOGLE "Server: GFE/"
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL(wval), tmp) {
...
static int pthreads_openssl_handle_ssl_error(pthreads_stream_t *threaded_stream, int nr_bytes, zend_bool is_init) { /* {{{ */
	pthreads_stream *stream = PTHREADS_FETCH_STREAMS_STREAM(threaded_stream);
	pthreads_openssl_netstream_data_t *sslsock;
	char esbuf[512];
	smart_str ebuf = {0};
	unsigned long ecode;
	int retry = 1;
	...
	if(stream_lock(threaded_stream)) {
		...
		err = SSL_get_error(sslsock->ssl_handle, nr_bytes);
		...
		switch(err) {
			...
			case SSL_ERROR_SYSCALL:
				if (ERR_peek_error() == 0) {
					if (nr_bytes == 0) {
						if (!pthreads_openssl_is_http_stream_talking_to_iis(threaded_stream) && ERR_get_error() != 0) {
...
static int pthreads_verify_callback(int preverify_ok, X509_STORE_CTX *ctx) { /* {{{ */
	pthreads_stream_t *threaded_stream;
	SSL *ssl;
	int err, depth, ret;
	zval *val;
	zend_ulong allowed_depth = PTHREADS_OPENSSL_DEFAULT_STREAM_VERIFY_DEPTH;
	...
	threaded_stream = (pthreads_stream_t*)SSL_get_ex_data(ssl, pthreads_openssl_get_ssl_stream_data_index());
...
static int pthreads_openssl_x509_fingerprint_cmp(X509 *peer, const char *method, const char *expected) {
	zend_string *fingerprint;
	int result = -1;

	fingerprint = pthreads_openssl_x509_fingerprint(peer, method, 0);
	if (fingerprint) {
...
static zend_bool pthreads_openssl_x509_fingerprint_match(X509 *peer, zval *val)
{
	if (Z_TYPE_P(val) == IS_STRING) {
		...
		return method && pthreads_openssl_x509_fingerprint_cmp(peer, method, Z_STRVAL_P(val)) == 0;
		...
		ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(val), key, current) {
			...
			if (pthreads_openssl_x509_fingerprint_cmp(peer, ZSTR_VAL(key), Z_STRVAL_P(current)) != 0) {
				return 0;
			}
		} ZEND_HASH_FOREACH_END();
...
static zend_bool pthreads_openssl_matches_wildcard_name(const char *subjectname, const char *certname) { /* {{{ */
	char *wildcard = NULL;
...
static zend_bool pthreads_openssl_matches_san_list(X509 *peer, const char *subject_name) { /* {{{ */
	int i, len;
	...
	for (i = 0; i < alt_name_count; i++) {
		...
		if (san->type == GEN_DNS) {
			...
			if (pthreads_openssl_matches_wildcard_name(subject_name, (const char *)cert_name)) {
				OPENSSL_free(cert_name);
				return 1;
			}
			OPENSSL_free(cert_name);
...
static zend_bool pthreads_openssl_matches_common_name(X509 *peer, const char *subject_name) { /* {{{ */
	char buf[1024];
	X509_NAME *cert_name;
	...
	} else if (pthreads_openssl_matches_wildcard_name(subject_name, buf)) {
		is_match = 1;
	} else {
		php_error_docref(NULL, E_WARNING,
...
static int pthreads_openssl_apply_peer_verification_policy(SSL *ssl, X509 *peer, pthreads_stream_t *threaded_stream) { /* {{{ */
	...
	if(stream_lock(threaded_stream)) {
		...
		if (must_verify_fingerprint) {
			if (Z_TYPE_P(peer_fingerprint) == IS_STRING || Z_TYPE_P(peer_fingerprint) == IS_ARRAY) {
				if (!pthreads_openssl_x509_fingerprint_match(peer, peer_fingerprint)) {
					stream_unlock(threaded_stream);
					php_error_docref(NULL, E_WARNING,
		...
		if (must_verify_peer_name) {
			...
			if (peer_name && (pthreads_openssl_matches_san_list(peer, peer_name) ||
					pthreads_openssl_matches_common_name(peer, peer_name))) {
				return SUCCESS;
			}
			return FAILURE;
		}
...
static int pthreads_openssl_passwd_callback(char *buf, int num, int verify, void *data) { /* {{{ */
	pthreads_stream_t *threaded_stream = (pthreads_stream_t *)data;
	zval *val = NULL;
...
static int pthreads_openssl_win_cert_verify_callback(X509_STORE_CTX *x509_store_ctx, void *arg) { /* {{{ */
	...
	threaded_stream = (pthreads_stream_t*)arg;
...
static long pthreads_openssl_load_stream_cafile(X509_STORE *cert_store, const char *cafile) { /* {{{ */
	pthreads_stream_t *threaded_stream;
	pthreads_stream *stream;
	X509 *cert;
	BIO *buffer;
	...
	threaded_stream = pthreads_stream_open_wrapper(cafile, "rb", 0, NULL);
	...
	if (threaded_stream == NULL) {
		...
		if (stream->wrapper != NULL && PTHREADS_FETCH_STREAMS_WRAPPER(stream->wrapper)->is_url) {
	...
	if(stream_lock(threaded_stream)) {
		cert_start: {
			line = pthreads_stream_get_line(threaded_stream, NULL, 0, &line_len);
			if (line == NULL) {
...
static int pthreads_openssl_enable_peer_verification(SSL_CTX *ctx, pthreads_stream_t *threaded_stream) { /* {{{ */
	zval *val = NULL;
	char *cafile = NULL;
	char *capath = NULL;
	pthreads_stream *stream;
	...
	if(stream_lock(threaded_stream)) {
		...
		if (cafile || capath) {
			if (!SSL_CTX_load_verify_locations(ctx, cafile, capath)) {
				if (cafile && !pthreads_openssl_load_stream_cafile(SSL_CTX_get_cert_store(ctx), cafile)) {
					stream_unlock(threaded_stream);
					return FAILURE;
				}
			}
...
#ifdef PHP_WIN32
			SSL_CTX_set_cert_verify_callback(ctx, pthreads_openssl_win_cert_verify_callback, (void *)threaded_stream);
		...
		SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER, pthreads_verify_callback);
...
static void pthreads_openssl_disable_peer_verification(SSL_CTX *ctx, pthreads_stream_t *threaded_stream) { /* {{{ */
	SSL_CTX_set_verify(ctx, SSL_VERIFY_NONE, NULL);
}
/* }}} */

static int pthreads_openssl_set_local_cert(SSL_CTX *ctx, pthreads_stream_t *threaded_stream) { /* {{{ */
	zval *val = NULL;
	char *certfile = NULL;
...
static char *pthreads_openssl_cipher_get_version(const SSL_CIPHER *c, char *buffer, size_t max_len) { /* {{{ */
...
#if PTHREADS_OPENSSL_API_VERSION < 0x10100
static int pthreads_openssl_get_crypto_method_ctx_flags(int method_flags) { /* {{{ */
...
#endif
#ifdef HAVE_SSL3
	if (!(method_flags & PTHREADS_STREAM_CRYPTO_METHOD_SSLv3)) {
		ssl_ctx_options |= SSL_OP_NO_SSLv3;
	}
#endif
	if (!(method_flags & PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_0)) {
		ssl_ctx_options |= SSL_OP_NO_TLSv1;
	}
#ifdef HAVE_TLS11
	if (!(method_flags & PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_1)) {
		ssl_ctx_options |= SSL_OP_NO_TLSv1_1;
	}
#endif
#ifdef HAVE_TLS12
	if (!(method_flags & PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_2)) {
...
static inline int pthreads_openssl_get_min_proto_version_flag(int flags) { /* {{{ */
	int ver;
	for (ver = PTHREADS_OPENSSL_MIN_PROTO_VERSION; ver <= PTHREADS_OPENSSL_MAX_PROTO_VERSION; ver <<= 1) {
		if (flags & ver) {
			return ver;
		}
	}
	return PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_2;
}
/* }}} */

static inline int pthreads_openssl_get_max_proto_version_flag(int flags) { /* {{{ */
	int ver;
	for (ver = PTHREADS_OPENSSL_MAX_PROTO_VERSION; ver >= PTHREADS_OPENSSL_MIN_PROTO_VERSION; ver >>= 1) {
		if (flags & ver) {
			return ver;
		}
	}
...
#if PTHREADS_OPENSSL_API_VERSION >= 0x10100
static inline int pthreads_openssl_map_proto_version(int flag) /* {{{ */
{
	switch (flag) {
#ifdef HAVE_SSL3
		case PTHREADS_STREAM_CRYPTO_METHOD_SSLv3:
			return SSL3_VERSION;
#endif
		case PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_0:
			return TLS1_VERSION;
		case PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_1:
...
static int pthreads_openssl_get_min_proto_version(int flags) { /* {{{ */
	return pthreads_openssl_map_proto_version(pthreads_openssl_get_min_proto_version_flag(flags));
}
/* }}} */

static int pthreads_openssl_get_max_proto_version(int flags) { /* {{{ */
	return pthreads_openssl_map_proto_version(pthreads_openssl_get_max_proto_version_flag(flags));
}
/* }}} */
#endif

static int pthreads_openssl_get_proto_version_flags(int flags, int min, int max) /* {{{ */
{
	int ver;

	if (!min) {
		min = pthreads_openssl_get_min_proto_version_flag(flags);
	}
	if (!max) {
		max = pthreads_openssl_get_max_proto_version_flag(flags);
	}
...
static void pthreads_openssl_limit_handshake_reneg(const SSL *ssl) { /* {{{ */
	pthreads_stream_t *threaded_stream;
	...
	threaded_stream = pthreads_openssl_get_stream_from_ssl_handle(ssl);
	...
	if(stream_lock(threaded_stream)) {
		...
		if (sslsock->reneg->tokens > sslsock->reneg->limit) {
			...
			if(threaded_context != NULL && (val = pthreads_stream_context_get_option(threaded_context,
					"ssl", "reneg_limit_callback")) != NULL
			) {
				zval param, retval;

				pthreads_stream_to_zval(threaded_stream, &param);

				/* Closing the stream inside this callback would segfault! */
				stream->flags |= PTHREADS_STREAM_FLAG_NO_FCLOSE;
...
static void pthreads_openssl_info_callback(const SSL *ssl, int where, int ret) { /* {{{ */
	/* Rate-limit client-initiated handshake renegotiation to prevent DoS */
	if (where & SSL_CB_HANDSHAKE_START) {
		pthreads_openssl_limit_handshake_reneg(ssl);
	}
}
/* }}} */

static void pthreads_openssl_init_server_reneg_limit(pthreads_stream_t *threaded_stream, pthreads_openssl_netstream_data_t *sslsock) { /* {{{ */
	pthreads_stream_context_t *threaded_context = pthreads_stream_get_context(threaded_stream);
	zval *val;
	zend_long limit = PTHREADS_OPENSSL_DEFAULT_RENEG_LIMIT;
	zend_long window = PTHREADS_OPENSSL_DEFAULT_RENEG_WINDOW;

	if(stream_lock(threaded_stream)) {
		if (threaded_context != NULL &&
			NULL != (val = pthreads_stream_context_get_option(threaded_context, "ssl", "reneg_limit"))
		) {
		...
		if (threaded_context != NULL &&
			NULL != (val = pthreads_stream_context_get_option(threaded_context, "ssl", "reneg_window"))
		) {
		...
		SSL_set_info_callback(sslsock->ssl_handle, pthreads_openssl_info_callback);
...
static int pthreads_openssl_set_server_dh_param(pthreads_stream_t * threaded_stream, SSL_CTX *ctx) { /* {{{ */
	DH *dh;
	BIO* bio;
	zval *zdhpath;

	//if (pthreads_stream_has_context(threaded_stream)) {
	zdhpath = pthreads_stream_context_get_option(pthreads_stream_get_context(threaded_stream), "ssl", "dh_param");
	if (zdhpath == NULL) {
#if 0
	...
	bio = BIO_new_file(Z_STRVAL_P(zdhpath), PTHREADS_OPENSSL_BIO_MODE_R(PKCS7_BINARY));
...
#if defined(HAVE_ECDH) && PTHREADS_OPENSSL_API_VERSION < 0x10100
static int pthreads_openssl_set_server_ecdh_curve(pthreads_stream_t *threaded_stream, SSL_CTX *ctx) { /* {{{ */
	zval *zvcurve;
	int curve_nid;
	EC_KEY *ecdh;

	zvcurve = pthreads_stream_context_get_option(pthreads_stream_get_context(threaded_stream), "ssl", "ecdh_curve");
	if (zvcurve == NULL) {
#if PTHREADS_OPENSSL_VERSION_NUMBER >= 0x10002000L
...
static int pthreads_openssl_set_server_specific_opts(pthreads_stream_t *threaded_stream, pthreads_stream_context_t *threaded_context, SSL_CTX *ctx) { /* {{{ */
	zval *zv;
	long ssl_ctx_options = SSL_CTX_get_options(ctx);

#if defined(HAVE_ECDH) && PTHREADS_OPENSSL_API_VERSION < 0x10100
	if (pthreads_openssl_set_server_ecdh_curve(threaded_stream, ctx) == FAILURE) {
		return FAILURE;
	}
#endif

#if PTHREADS_OPENSSL_API_VERSION < 0x10100
	SSL_CTX_set_tmp_rsa_callback(ctx, pthreads_openssl_tmp_rsa_cb);
#endif
	/* We now use php_openssl_tmp_rsa_cb to generate a key of appropriate size whenever necessary */
	if (pthreads_stream_context_get_option(threaded_context, "ssl", "rsa_key_size") != NULL) {
		php_error_docref(NULL, E_WARNING, "rsa_key_size context option has been removed");
	}

	pthreads_openssl_set_server_dh_param(threaded_stream, ctx);
	zv = pthreads_stream_context_get_option(threaded_context, "ssl", "single_dh_use");
	if (zv == NULL || zend_is_true(zv)) {
		ssl_ctx_options |= SSL_OP_SINGLE_DH_USE;
	}

	zv = pthreads_stream_context_get_option(threaded_context, "ssl", "honor_cipher_order");
	if (zv == NULL || zend_is_true(zv)) {
...
#ifdef HAVE_TLS_SNI
static int pthreads_openssl_server_sni_callback(SSL *ssl_handle, int *al, void *arg) { /* {{{ */
	pthreads_stream_t *threaded_stream;
	pthreads_stream *stream;
	pthreads_openssl_netstream_data_t *sslsock;
	unsigned i;
	...
	threaded_stream = (pthreads_stream_t*)SSL_get_ex_data(ssl_handle, pthreads_openssl_get_ssl_stream_data_index());
	...
	if(stream_lock(threaded_stream)) {
		...
		for (i=0; i < sslsock->sni_cert_count; i++) {
			if (pthreads_openssl_matches_wildcard_name(server_name, sslsock->sni_certs[i].name)) {
...
static SSL_CTX *pthreads_openssl_create_sni_server_ctx(char *cert_path, char *key_path) { /* {{{ */
...
static int pthreads_openssl_enable_server_sni(pthreads_stream_t *threaded_stream, pthreads_openssl_netstream_data_t *sslsock) { /* {{{ */
	zval *val;
	zval *current;
	zend_string *key;
	...
	sslsock->sni_certs = (pthreads_openssl_sni_cert_t*)calloc(sslsock->sni_cert_count,
		sizeof(pthreads_openssl_sni_cert_t)
	);
	memset(sslsock->sni_certs, 0, sslsock->sni_cert_count * sizeof(pthreads_openssl_sni_cert_t));
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(val), key_index, key, current) {
		...
		if (Z_TYPE_P(current) == IS_ARRAY) {
			...
			ctx = pthreads_openssl_create_sni_server_ctx(resolved_cert_path_buff, resolved_pk_path_buff);

		} else if (VCWD_REALPATH(Z_STRVAL_P(current), resolved_path_buff)) {
			ctx = pthreads_openssl_create_sni_server_ctx(resolved_path_buff, resolved_path_buff);
		} else {
			php_error_docref(NULL, E_WARNING,
				"failed setting local cert chain file `%s'; file not found",
				Z_STRVAL_P(current)
		...
		sslsock->sni_certs[i].name = strdup(ZSTR_VAL(key));
		sslsock->sni_certs[i].ctx = ctx;
		++i;

	} ZEND_HASH_FOREACH_END();

	SSL_CTX_set_tlsext_servername_callback(sslsock->ctx, pthreads_openssl_server_sni_callback);

	return SUCCESS;
}
/* }}} */

static void pthreads_openssl_enable_client_sni(pthreads_stream_t *threaded_stream, pthreads_openssl_netstream_data_t *sslsock) /* {{{ */
{
	zval *val;
 ...
 */
static unsigned char *pthreads_openssl_alpn_protos_parse(unsigned short *outlen, const char *in) { /* {{{ */
	size_t len;
	unsigned char *out;
	size_t i, start = 0;
...
static int pthreads_openssl_server_alpn_callback(SSL *ssl_handle,
...
int pthreads_openssl_setup_crypto(pthreads_stream_t *threaded_stream,
		pthreads_openssl_netstream_data_t *sslsock,
		pthreads_stream_xport_crypto_param *cparam) { /* {{{ */
	pthreads_stream_context_t *threaded_context = pthreads_stream_get_context(threaded_stream);
	pthreads_stream *stream;
	...
	if(stream_lock(threaded_stream)) {
		...
		sslsock->is_client = cparam->inputs.method & PTHREADS_STREAM_CRYPTO_IS_CLIENT;
		...
		if (alpn_protocols) {
#ifdef HAVE_TLS_ALPN
			{
				...
				unsigned char *alpn = pthreads_openssl_alpn_protos_parse(&alpn_len, alpn_protocols);
				...
				if (sslsock->is_client) {
					...
					sslsock->alpn_ctx.data = (unsigned char *) strndup((const char*)alpn, alpn_len);
					sslsock->alpn_ctx.len = alpn_len;
					SSL_CTX_set_alpn_select_cb(sslsock->ctx, pthreads_openssl_server_alpn_callback, sslsock);
				}
		...
		if (FAILURE == pthreads_openssl_set_local_cert(sslsock->ctx, threaded_stream)) {
			stream_unlock(threaded_stream);
			return FAILURE;
		}
...
#if PTHREADS_OPENSSL_API_VERSION >= 0x10100
		SSL_CTX_set_min_proto_version(sslsock->ctx, pthreads_openssl_get_min_proto_version(method_flags));
		SSL_CTX_set_max_proto_version(sslsock->ctx, pthreads_openssl_get_max_proto_version(method_flags));
#endif

		if (sslsock->is_client == 0 && threaded_context != NULL &&
			FAILURE == pthreads_openssl_set_server_specific_opts(threaded_stream, threaded_context, sslsock->ctx)
		) {
			stream_unlock(threaded_stream);
			return FAILURE;
		}
		...
		if (sslsock->ssl_handle == NULL) {
			...
			SSL_set_ex_data(sslsock->ssl_handle, pthreads_openssl_get_ssl_stream_data_index(), threaded_stream);
		}

		if (!SSL_set_fd(sslsock->ssl_handle, sslsock->s.socket)) {
			pthreads_openssl_handle_ssl_error(threaded_stream, 0, 1);
		}

#ifdef HAVE_TLS_SNI
		/* Enable server-side SNI */
		if (!sslsock->is_client && pthreads_openssl_enable_server_sni(threaded_stream, sslsock) == FAILURE) {
			stream_unlock(threaded_stream);
			return FAILURE;
		}
#endif
		...
		if (!sslsock->is_client) {
			pthreads_openssl_init_server_reneg_limit(threaded_stream, sslsock);
		}
		...
		if (cparam->inputs.session) {
			stream = PTHREADS_FETCH_STREAMS_STREAM(cparam->inputs.session);
			if (stream->ops != &pthreads_openssl_socket_ops) {
				php_error_docref(NULL, E_WARNING, "supplied session stream must be an SSL enabled stream");
			} else if (((pthreads_openssl_netstream_data_t*)stream->abstract)->ssl_handle == NULL) {
				php_error_docref(NULL, E_WARNING, "supplied SSL session stream is not initialized");
			} else {
				SSL_copy_session_id(sslsock->ssl_handle, ((pthreads_openssl_netstream_data_t*)stream->abstract)->ssl_handle);
			}
		}
		stream_unlock(threaded_stream);
	}
...
static zend_array *pthreads_openssl_capture_session_meta(SSL *ssl_handle) { /* {{{ */
	zval meta_arr;
	char *proto_str;
	long proto = SSL_version(ssl_handle);
	const SSL_CIPHER *cipher = SSL_get_current_cipher(ssl_handle);
	char version_str[PTHREADS_SSL_MAX_VERSION_LEN];
	...
	add_assoc_string(&meta_arr, "cipher_version",
			pthreads_openssl_cipher_get_version(cipher, version_str, PTHREADS_SSL_MAX_VERSION_LEN));

	return Z_ARR(meta_arr);
}
/* }}} */

static int pthreads_openssl_capture_peer_certs(pthreads_stream_t *threaded_stream, pthreads_stream_context_t *threaded_context, pthreads_openssl_netstream_data_t *sslsock, X509 *peer_cert) { /* {{{ */
	zval *val, zcert;
	int cert_captured = 0;

	if (NULL != (val = pthreads_stream_context_get_option(threaded_context, "ssl", "capture_peer_cert")) &&
		zend_is_true(val)
	) {
		ZVAL_OBJ(&zcert, PTHREADS_STD_P(pthreads_openssl_x509_new(peer_cert)));
		pthreads_stream_context_set_option(threaded_context, "ssl", "peer_certificate", &zcert);
		zval_ptr_dtor(&zcert);
		cert_captured = 1;
	}

	if (NULL != (val = pthreads_stream_context_get_option(threaded_context, "ssl", "capture_peer_cert_chain")) &&
		zend_is_true(val)
	) {
		zval arr;
		STACK_OF(X509) *chain;
		...
		if (chain && sk_X509_num(chain) > 0) {
			...
			for (i = 0; i < sk_X509_num(chain); i++) {
				...
				ZVAL_OBJ(&zcert, PTHREADS_STD_P(pthreads_openssl_x509_new(mycert)));
		...
		pthreads_stream_context_set_option(threaded_context, "ssl", "peer_certificate_chain", &arr);
		zval_ptr_dtor(&arr);
	}
...
static int pthreads_openssl_enable_crypto(pthreads_stream_t *threaded_stream,
		pthreads_openssl_netstream_data_t *sslsock,
		pthreads_stream_xport_crypto_param *cparam) { /* {{{ */
	int n;
	int retry = 1;
		...
		if (cparam->inputs.activate && !sslsock->ssl_active) {
	...
	#ifdef HAVE_TLS_SNI
			if (sslsock->is_client) {
				pthreads_openssl_enable_client_sni(threaded_stream, sslsock);
			}
	#endif
			...
			do {
				...
				if (has_timeout) {
					...
					elapsed_time = pthreads_openssl_subtract_timeval(cur_time, start_time);

					if (pthreads_openssl_compare_timeval( elapsed_time, *timeout) > 0) {
				...
				if (n <= 0) {
					...
					retry = pthreads_openssl_handle_ssl_error(threaded_stream, n, blocked);
					if (retry) {
						...
						if (has_timeout) {
							left_time = pthreads_openssl_subtract_timeval(*timeout, elapsed_time);
						}
			...
			if (n == 1) {
				...
				if (peer_cert && threaded_context != NULL) {
					cert_captured = pthreads_openssl_capture_peer_certs(threaded_stream, threaded_context, sslsock, peer_cert);
				}

				if (FAILURE == pthreads_openssl_apply_peer_verification_policy(sslsock->ssl_handle, peer_cert, threaded_stream)) {
...
static size_t pthreads_openssl_sockop_read(pthreads_stream_t *threaded_stream, char *buf, size_t count) { /* {{{ */
	return pthreads_openssl_sockop_io(1, threaded_stream, buf, count );
}
/* }}} */

static size_t pthreads_openssl_sockop_write(pthreads_stream_t *threaded_stream, const char *buf, size_t count) { /* {{{ */
	return pthreads_openssl_sockop_io(0, threaded_stream, (char*)buf, count );
}
/* }}} */
...
static size_t pthreads_openssl_sockop_io(int read, pthreads_stream_t *threaded_stream, char *buf, size_t count) { /* {{{ */
	...
	if(stream_lock(threaded_stream)) {
		pthreads_openssl_netstream_data_t *sslsock = (pthreads_openssl_netstream_data_t*)stream->abstract;
		...
		if (sslsock->ssl_active) {
			...
			do {
				...
				if (read) {
					...
					if (sslsock->reneg->should_close) {
						/* renegotiation rate limiting triggered */
						pthreads_stream_xport_shutdown(threaded_stream, (pthreads_stream_shutdown_t)PTHREADS_STREAM_SHUT_RDWR);
						nr_bytes = 0;
						stream->eof = 1;
						 break;
					}
				...
				if (nr_bytes <= 0) {
					...
					retry = pthreads_openssl_handle_ssl_error(threaded_stream, nr_bytes, 0);
			...
			if (read) {
				nr_bytes = pthreads_stream_socket_ops.read(threaded_stream, buf, count);
			} else {
				nr_bytes = pthreads_stream_socket_ops.write(threaded_stream, buf, count);
			}
			stream_unlock(threaded_stream);
...
static int pthreads_openssl_sockop_close(pthreads_stream_t *threaded_stream, int close_handle) { /* {{{ */
	pthreads_stream *stream = PTHREADS_FETCH_STREAMS_STREAM(threaded_stream);
	pthreads_openssl_netstream_data_t *sslsock;
...
static void pthreads_openssl_sockop_free(pthreads_stream_t *threaded_stream, int close_handle) { /* {{{ */
	pthreads_stream *stream = PTHREADS_FETCH_STREAMS_STREAM(threaded_stream);
	pthreads_openssl_netstream_data_t *sslsock = (pthreads_openssl_netstream_data_t*)stream->abstract;
	unsigned i;
	...
	if (sslsock->sni_certs) {
		for (i = 0; i < sslsock->sni_cert_count; i++) {
			if (sslsock->sni_certs[i].ctx) {
				...
				free(sslsock->sni_certs[i].name);
			}
		}
		free(sslsock->sni_certs);
		sslsock->sni_certs = NULL;
	}

	if (sslsock->url_name) {
		free(sslsock->url_name);
	}
...
static int pthreads_openssl_sockop_flush(pthreads_stream_t *threaded_stream) { /* {{{ */
	return pthreads_stream_socket_ops.flush(threaded_stream);
}
/* }}} */

static int pthreads_openssl_sockop_stat(pthreads_stream_t *threaded_stream, pthreads_stream_statbuf *ssb) { /* {{{ */
	return pthreads_stream_socket_ops.stat(threaded_stream, ssb);
}
/* }}} */

static inline int pthreads_openssl_tcp_sockop_accept(pthreads_stream_t *threaded_stream, pthreads_openssl_netstream_data_t *sock,
	...
	if (threaded_context != NULL && (tmpzval = pthreads_stream_context_get_option(threaded_context, "socket", "tcp_nodelay")) != NULL &&
	...
	if (clisock >= 0) {
		pthreads_openssl_netstream_data_t *clisockdata = (pthreads_openssl_netstream_data_t*) malloc(sizeof(*clisockdata));
		...
		xparam->outputs.client = PTHREADS_STREAM_CLASS_NEW(stream->ops, clisockdata, "r+", pthreads_socket_stream_entry);
		if (xparam->outputs.client && threaded_context != NULL) {
			pthreads_stream_set_context(xparam->outputs.client, threaded_context);
		}

		if (xparam->outputs.client && sock->enable_on_connect) {
			/* remove the client bit */
			if (sock->method & PTHREADS_STREAM_CRYPTO_IS_CLIENT) {
				sock->method = ((sock->method >> 1) << 1);
			}

			clisockdata->method = sock->method;

			if (pthreads_stream_xport_crypto_setup(xparam->outputs.client, clisockdata->method, NULL) < 0 ||
					pthreads_stream_xport_crypto_enable(xparam->outputs.client, 1) < 0) {
				php_error_docref(NULL, E_WARNING, "Failed to enable crypto");

				pthreads_stream_close(xparam->outputs.client, PTHREADS_STREAM_FREE_CLOSE);
				xparam->outputs.client = NULL;
...
static int pthreads_openssl_sockop_set_option(pthreads_stream_t *threaded_stream, int option, int value, void *ptrparam) { /* {{{ */
	pthreads_stream *stream = PTHREADS_FETCH_STREAMS_STREAM(threaded_stream);
	pthreads_openssl_netstream_data_t *sslsock;
	pthreads_stream_xport_crypto_param *cparam = (pthreads_stream_xport_crypto_param *)ptrparam;
	pthreads_stream_xport_param *xparam = (pthreads_stream_xport_param *)ptrparam;
	...
	if(stream_lock(threaded_stream)) {
		...
		switch (option) {
			...
			case PTHREADS_STREAM_OPTION_CRYPTO_API:

				switch(cparam->op) {

					case PTHREADS_STREAM_XPORT_CRYPTO_OP_SETUP:
						cparam->outputs.returncode = pthreads_openssl_setup_crypto(threaded_stream, sslsock, cparam);
						stream_unlock(threaded_stream);
						return PTHREADS_STREAM_OPTION_RETURN_OK;
						break;
					case PTHREADS_STREAM_XPORT_CRYPTO_OP_ENABLE:
						cparam->outputs.returncode = pthreads_openssl_enable_crypto(threaded_stream, sslsock, cparam);
						stream_unlock(threaded_stream);
						return PTHREADS_STREAM_OPTION_RETURN_OK;
						break;
					default:
			...
			case PTHREADS_STREAM_OPTION_XPORT_API:
				switch(xparam->op) {
					...
					case PTHREADS_STREAM_XPORT_OP_CONNECT_ASYNC:
						...
						pthreads_stream_socket_ops.set_option(threaded_stream, option, value, ptrparam);
						...
						{
							if (pthreads_stream_xport_crypto_setup(threaded_stream, sslsock->method, NULL) < 0 ||
					...
					case PTHREADS_STREAM_XPORT_OP_ACCEPT:
						 ...
						 * doesn't know about */
						xparam->outputs.returncode = pthreads_openssl_tcp_sockop_accept(threaded_stream, sslsock, xparam);
	...
	return pthreads_stream_socket_ops.set_option(threaded_stream, option, value, ptrparam);
}
/* }}} */

static int pthreads_openssl_sockop_cast(pthreads_stream_t *threaded_stream, int castas, void **ret) { /* {{{ */
	pthreads_stream *stream = PTHREADS_FETCH_STREAMS_STREAM(threaded_stream);
	pthreads_openssl_netstream_data_t *sslsock;
				...
				if (ret) {
					...
					if (stream->writepos == stream->readpos
						...
						&& (pending = (size_t)SSL_pending(sslsock->ssl_handle)) > 0) {
							pthreads_stream_fill_read_buffer(threaded_stream, pending < stream->chunk_size
								? pending
								: stream->chunk_size);
					}
...
const pthreads_stream_ops pthreads_openssl_socket_ops = {
	pthreads_openssl_sockop_write, pthreads_openssl_sockop_read,
	pthreads_openssl_sockop_close, pthreads_openssl_sockop_free,
	pthreads_openssl_sockop_flush,
	"tcp_socket/ssl",
	NULL, /* seek */
	pthreads_openssl_sockop_cast,
	pthreads_openssl_sockop_stat,
	pthreads_openssl_sockop_set_option,
};

static zend_long pthreads_openssl_get_crypto_method(pthreads_stream_context_t *threaded_ctx, zend_long crypto_method) { /* {{{ */
	zval *val;

	if (threaded_ctx && (val = pthreads_stream_context_get_option(threaded_ctx, "ssl", "crypto_method")) != NULL) {
		crypto_method = zval_get_long(val);
		crypto_method |= PTHREADS_STREAM_CRYPTO_IS_CLIENT;
	}
...
static char *pthreads_openssl_get_url_name(const char *resourcename, size_t resourcenamelen) { /* {{{ */
	pthreads_url *url;
	...
	url = pthreads_url_parse_ex(resourcename, resourcenamelen);
	if (!url) {
		return NULL;
	}
	...
	if (url->host) {
		...
		if (len) {
			url_name = strndup(host, len);
		}

		pthreads_url_free(url);
		return url_name;
	}
...
pthreads_stream_t *pthreads_openssl_ssl_socket_factory(const char *proto, size_t protolen,
		const char *resourcename, size_t resourcenamelen,
		int options, int flags, struct timeval *timeout,
		pthreads_stream_context_t *threaded_context) { /* {{{ */
	pthreads_stream_t *threaded_stream = NULL;
	pthreads_openssl_netstream_data_t *sslsock = NULL;

	sslsock = calloc(1, sizeof(pthreads_openssl_netstream_data_t));

	sslsock->reneg = (void*)malloc(sizeof(pthreads_openssl_handshake_bucket_t));
	sslsock->reneg->should_close = 0;
	...
	threaded_stream = PTHREADS_STREAM_CLASS_NEW(&pthreads_openssl_socket_ops, sslsock, "r+", pthreads_socket_stream_entry);
	...
	if (strncmp(proto, "ssl", protolen) == 0) {
		...
		sslsock->method = pthreads_openssl_get_crypto_method(threaded_context, PTHREADS_STREAM_CRYPTO_METHOD_TLS_ANY_CLIENT);
...
#ifdef HAVE_SSL3
		sslsock->enable_on_connect = 1;
		sslsock->method = PTHREADS_STREAM_CRYPTO_METHOD_SSLv3_CLIENT;
#else
		php_error_docref(NULL, E_WARNING,
		...
		sslsock->enable_on_connect = 1;
		sslsock->method = PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_0_CLIENT;
	} else if (strncmp(proto, "tlsv1.1", protolen) == 0) {
#ifdef HAVE_TLS11
		sslsock->enable_on_connect = 1;
		sslsock->method = PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_1_CLIENT;
#else
		php_error_docref(NULL, E_WARNING,
...
#ifdef HAVE_TLS12
		sslsock->enable_on_connect = 1;
		sslsock->method = PTHREADS_STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT;
#else
		php_error_docref(NULL, E_WARNING,
	...
	sslsock->url_name = pthreads_openssl_get_url_name(resourcename, resourcenamelen);
