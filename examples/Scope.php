...
/*
 * Method Scope 101 protected: 	A protected method may only be called by one thread at a time 	If
 * Thread A is calling a protected method and Thread B attempts to call the same method Thread B
 * will block until Thread A has left the method private: 	private with respect to the scope of the
 * Thread, ie: you can only call private methods from within the threading context
 */
...
class ExampleThread extends Thread
{
    ...
    protected function exclusive($arg = null)
    {
        ...
        if ($arg) {
            ...
            $result = sprintf("%s: got nothing", __METHOD__);
        }
        printf("%s: %s\n", __METHOD__, microtime(true));
        usleep(1000000);
        printf("OUT->%s: %s\n", __METHOD__, microtime(true));
...
// Passing an argument on the command line will show you what happens when you call a private method
// from here
if (isset($argv[1])) {
    printf("Process: %s\n", $thread->noaccess());
}
